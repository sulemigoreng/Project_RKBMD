...
$objPHPExcel->getActiveSheet()
			->setCellValue('C2', 'USULAN RENCANA KEBUTUHAN BARANG MILIK DAERAH')
...
$objPHPExcel->getActiveSheet()
			->setCellValue('C10','No.')
			->setCellValue('D10','Program/Kegiatan/Output')
			->setCellValue('H10','Usulan BMD')
...
$objPHPExcel->getActiveSheet() //Merge
			...
			->mergeCells('R10:S10')
			->mergeCells('D12:G12');
$objPHPExcel->getActiveSheet()->getStyle('C10:T12')->applyFromArray($style);
...
while($dataProgram=mysqli_fetch_array($queryProgram)){
	...
	while($dataKegiatan=mysqli_fetch_array($queryKegiatan)){
		...
		$queryUsulan=mysqli_query($koneksi,"SELECT b.kode_barang,nama_barang,jumlah,a.satuan,kuantitas_maks 
		...
		while($dataUsulan=mysqli_fetch_array($queryUsulan)){
			...
			if($dataOptimasi=mysqli_fetch_array($queryOptimasi)){
				$objPHPExcel->getActiveSheet()
						...
						->setCellValue('T'.$noCell , $dataOptimasi['keterangan']);
			}else{
				$objPHPExcel->getActiveSheet()
...
$objPHPExcel->getActiveSheet()->getStyle('C10:T'.$noCell)->applyFromArray($styleBorder);
...
$objPHPExcel->getActiveSheet()
			...
			->setCellValue('P'.(++$noCell),'Pengguna Barang')
			->mergeCells('P'.$noCell.':Q'.$noCell)
			->setCellValue('P'.(++$noCell),'Badan Pengelola Keuangan Dan Aset Daerah')
